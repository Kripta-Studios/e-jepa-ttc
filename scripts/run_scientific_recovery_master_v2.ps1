...
Set-Location $Root
if($NumWorkers -ne 0){
    throw "Scientific Recovery V2 requires -NumWorkers 0 with the current ~670 MiB torch cache shards. Re-shard before increasing worker count."
}

$ExpectedBase = "954fa52"
$Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$Master = Join-Path $Root "artifacts\scientific_recovery_master_v2"
...
$OutputZip = Join-Path $Root "artifacts\audit\E_JEPA_SCIENTIFIC_RECOVERY_MASTER_V2_RESULTS_$Timestamp.zip"
...
function Write-Status {
    ...
    $payload = [ordered]@{
        artifact_type = "scientific_recovery_master_status_v2"
...
function Invoke-SequentialTrainPair {
    ...
    Add-StepStatus "${Prefix}_execution_policy" "hardware" "SEQUENTIAL" 0 "V2 forbids concurrent CUDA training on 12GB WDDM after V1 OOM" $Logs
...
try {
            ...
            if($sub -eq 0){
                ...
                if($bc -eq 0){
                    ...
                    if($gt -eq 0 -and (Test-Path $candidatePred) -and (Test-Path $garlPred)){
                        ...
                        $pb=Invoke-Python "73_paired_bootstrap" "garl_compare" @("scripts/paired_cluster_bootstrap.py","--ejepa-predictions",$candidatePred,"--garl-predictions",$garlPred,"--resamples",[string]$BootstrapResamples,"--seed","20260811","--output",$paired) -AllowFailure
                        if($pb -ne 0 -or -not(Test-Path $paired)){$script:ClaimsBlocked=$true}
                    } else {$script:ClaimsBlocked=$true}
                } else {$script:ClaimsBlocked=$true}
            } else {$script:ClaimsBlocked=$true}
    ...
    } else {$script:ClaimsBlocked=$true; Add-StepStatus "70_garl_budget_matched" "garl_compare" "SKIPPED" 0 "-SkipBudgetMatchedGarl requested; SOTA comparison remains blocked" $Audit}
