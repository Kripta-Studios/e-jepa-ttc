...
$Out = "artifacts\debug\object_event_v4_26_oof_residual_stack"
...
$V425Summary = "artifacts\debug\object_event_v4_25_geometry_readout\summary.json"
...
$Preflight = @(
    "scripts\preflight_object_event_v4_26.py",
    "--v424-summary", $V424Summary,
    "--v425-summary", $V425Summary
)
...
if ($LASTEXITCODE -ne 0) { throw "v4.26 preflight failed with exit code $LASTEXITCODE" }

$Args = @(
    "scripts\analyze_object_event_v4_26_oof_residual_stack.py",
...
if ($LASTEXITCODE -ne 0) { throw "v4.26 analysis failed with exit code $LASTEXITCODE" }
Write-Host "v4.26 completed. Inspect summary.json, train_only_residual_ranking.csv and validation_per_track.csv."
exit 0
