...
def _base_mutate(source: dict[str, Any], arm: str, seed: int, num_workers: int, prefetch_factor: int) -> dict[str, Any]:
...
def main() -> int:
    ...
    for seed in (7,13,23):
        ...
        a4_path=out/f"a4_s1_lambda8_causal_left_seed{seed}.yaml"; _write(a4_path,a4)
    ...
    for seed in (7,13,23):
        ...
        if args.winner_stage in {"a6","a7"}:
            ...
            if cp_sha is not None: tr["initialization_checkpoint_sha256"] = cp_sha
            ...
            if isinstance(dc.get(key),dict):
                ...
                if cp_sha is not None: dc[key]["initialization_checkpoint_sha256"] = cp_sha
        path=out/f"{args.winner_stage}_s1_causal_left_seed{seed}.yaml"; _write(path,w)
    ...
    manifest={
        ...
        "contract":{"model_prefix_causal":True,"oracle_roi_preprocessing":True,"private_test_opened":False},
    }
