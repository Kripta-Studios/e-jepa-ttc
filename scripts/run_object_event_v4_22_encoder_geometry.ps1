...
$Out = "artifacts\debug\object_event_v4_22_encoder_geometry"
...
$V421 = "artifacts\debug\object_event_v4_21_box_pseudoflow_target_audit\summary.json"
...
$Config = "configs\experiment\e_jepa_garl_object_event_encoder_pseudoflow_v4_22.yaml"
...
$PreflightArgs = @(
    "scripts\preflight_object_event_v4_22.py",
...
if ($LASTEXITCODE -ne 0) { throw "v4.22 preflight failed with exit code $LASTEXITCODE" }

$Args = @(
    "scripts\analyze_object_event_v4_22_encoder_geometry.py",
    "--cache-manifest", $Cache,
...
if ($LASTEXITCODE -ne 0) { throw "v4.22 encoder pseudoflow experiment failed with exit code $LASTEXITCODE" }
Write-Host "v4.22 completed. Inspect summary.decision; no scientific pass/fail exit gate."
