...
def main() -> None:
    ...
    if runs_dir.exists():
        for run_path in runs_dir.iterdir():
            ...
            if args.require_full_label and float(metrics.get("train_fraction", metrics.get("label_fraction", 0.0))) < 1.0:
            ...
            if nav_mode is None and "uses_ego_actions_in_student_predictor" in metrics:
                nav_mode = "enabled" if metrics["uses_ego_actions_in_student_predictor"] else "disabled"
            ...
            run_model = metrics.get("model_name", metrics.get("method", "event-tubelet-transformer"))
            ...
            if metrics.get("final_test_opened") is True or metrics.get("evaluation_split") == "test":
                print(json.dumps({"run": run_path.name, "status": "rejected", "reason": "final_test_exposure"}), file=sys.stderr)
                continue
            
            # Fetch mae
            mae = None
            if "splits" in metrics and "validation" in metrics["splits"] and "metrics" in metrics["splits"]["validation"] and "mae_s" in metrics["splits"]["validation"]["metrics"]:
            ...
            if not ckpt_path_str:
                print(json.dumps({"run": run_path.name, "status": "rejected", "reason": "missing_checkpoint_path_in_manifest"}), file=sys.stderr)
                continue
            ...
            try:
                ...
                print(json.dumps({"run": run_path.name, "status": "rejected", "reason": "checkpoint_outside_run_directory"}), file=sys.stderr)
                continue
                
            cache_path = metrics.get("cache_manifest", metrics.get("cache", metrics.get("cache_path")))
            cache_sha256 = metrics.get("cache_sha256", metrics.get("manifest_sha256"))
            ...
            if mae < best_mae:
                ...
                best_record = {
                    ...
                    "cache_sidecar_sha256": metrics.get("cache_sidecar_sha256", "fake_hash"),
                    "model_config": model_config_dict,
                    "model_config_sha256": cfg_sha256,
                    "split_manifest_sha256": metrics.get("split_manifest_sha256", cache_sha256),
                    "normalization_sha256": metrics.get("normalization_sha256", "fake_hash"),
