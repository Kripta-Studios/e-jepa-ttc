"""Run the disclosed CARLA JEPA + synthetic-TTC auxiliary ablation."""
...
def _profile(name: str) -> CarlaJEPATrainerConfig:
    if name == "smoke":
        return CarlaJEPATrainerConfig(
            ...
            synthetic_ttc_loss_weight=1.0,
        )
    return CarlaJEPATrainerConfig(synthetic_ttc_loss_weight=1.0)


def main() -> int:
    """Parse a budget-matched CARLA synthetic-TTC ablation."""
    ...
    config = replace(
        config,
        synthetic_ttc_loss_weight=args.ttc_loss_weight,
        **{key: value for key, value in overrides.items() if value is not None},
    )
    output = args.output or Path(f"artifacts/runs/carla_jepa_ttc_{args.profile}_seed{config.seed}")
    if args.dry_run:
