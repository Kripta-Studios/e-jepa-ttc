...
def test_synthetic_pipeline_smoke(tmp_path: Path):
        ...
        json.dump(
            {
                "version": "1.0",
                "shards": [
                    {"path": str(cache_path), "split": "train", "sequence_ids": ["seq1", "seq2"]},
                    {"path": str(cache_path), "split": "validation", "sequence_ids": ["seq3"]},
                ],
                "splits": {"train": ["seq1", "seq2"], "validation": ["seq3"]},
            },
            f,
        )
    ...
    script_path = (
        Path(__file__).resolve().parent.parent.parent / "scripts" / "run_object_jepa_matrix.py"
    )
