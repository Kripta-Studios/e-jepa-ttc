...
def test_calibration_metrics_come_from_calibration():
    ...
    with pytest.raises(KeyError, match="median_abs_error_s"):
        float(calibration_metrics["median_abs_error_s"])
...
def test_no_default_zeros_in_calibration():
    ...
    with open(object_jepa_path, encoding="utf-8") as f:
        content = f.read()
    ...
    assert '.get("median_abs_error_s", 0.0)' not in content, (
        "Found forbidden .get with default zero for median_abs_error_s"
    )
